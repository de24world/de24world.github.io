...
======================
# 1. C언어 ? 
* C언어의 경우 1972년에 만들어졌으며, 오래 전에 만들어진 언어인만큼 무수한 다른 언어에게 영향을 미쳤다. C가 만들어진 1970년대에는 컴푸타가 지금처러 빠르지 않았으며, 그러한 이유로 C는 리소스가 많이 없는 기계에서 작동할 때 유용하며, 하드웨어와 매우 밀접하게 작동한다. 예를 들면 화상탐사선에서 사진을 찌고 NASA로 전송하는 역할을 하는데 250만 줄의 C언어의 코딩으로 작성되어있다고 한다. 왜냐하면 화상탐사선에는 프로세싱 파워도 약하고, 가능한 적게 공간을 쓰면서 빠른 언어가 필요했기 때문이다.
...
## ○ 참고 영상
* [C. C++. C#. 차이점 알려드림. 5분 순삭.](https://youtu.be/zGrTT4k1-yc)
* [빌드(Build)를 위한 Jenkins 설치 및 설정하기](https://youtu.be/m0tky1jyP-0)


## ○ 참조 문서 및 사이트
* [클라우드 컴퓨팅, IaaS, PaaS, SaaS이란?](https://reference-m1.tistory.com/221) 
